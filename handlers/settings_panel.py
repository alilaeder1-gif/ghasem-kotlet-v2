...
async def show_welcome_settings(msg_or_cq, chat_id):
    ...
    text = (
        ...
        f"╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌\n"
        f"وضعیت: {'🟢 فعال' if enabled else '🔴 غیرفعال'}\n\n"
        f"وقتی کسی جوین میشه، بات به صورت خودکار اسمش رو توی متن قرار میده.\n\n"
        f"📝 **متن فعلی:**\n"
        f"`{msg_text[:200]}`\n\n"
        f"📌 **متغیرهای اختیاری:**\n"
        f"• `{{name}}` ← اسم کاربر رو خودکار میذاره\n"
        f"• `{{group}}` ← اسم گروه رو خودکار میذاره\n"
        f"• `{{id}}` ← آیدی کاربر رو خودکار میذاره\n\n"
        f"👇 **یک قالب آماده انتخاب کن** یا خودت بنویس:"
    )
    WELCOME_TEMPLATES = [
...
async def settings_panel_cb(cq: CallbackQuery):
    ...
    if action == "welcome_edit":
        ...
        await cq.message.reply(
            f"✏️ **تنظیم متن خوشامد دلخواه**\n"
            f"╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌\n\n"
            f"همین حالا متن خوشامدت رو بنویس و بفرست.\n"
            f"بات به طور خودکار اسم کاربر رو جایگزین میکنه.\n\n"
            f"**قابل استفاده در متن:**\n"
            f"• `{{name}}` ← اسم کاربر (مثال: علی)\n"
            f"• `{{group}}` ← اسم گروه\n"
            f"• `{{id}}` ← آیدی عددی کاربر\n\n"
            f"**مثال:**\n"
            f"`سلام به گروه خوش اومدی` ← بدون نام\n"
            f"`سلام {name} به گروه خوش اومدی` ← با نام\n\n"
